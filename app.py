...
app.config["CACHE_DEFAULT_TIMEOUT"] = "10000"
cache=Cache(app)
...
# @app.route("/<string:username>/report")
# def hello_world(username):
#   return render_template("Report_message.html")
  ...
  for i in user_data:
    ...
    for j in decks_data:
      if i.user_name==j.user_name:
        if j.last_access!=datecalc():
          not_revised.append(j.deck_name)

    remove_redundancy=set(not_revised)
    new_not_revised=list(remove_redundancy)
    if (len(new_not_revised)==0):
      send_data="Hi "+i.first_name+" Congrats You have revised today"
    else:
      send_data="Hi "+i.first_name+" Kindly revise: "
      for k in new_not_revised:
        send_data+=k
      

    payload={'text' : send_data}
    r=requests.post(webhookurl,data=str(payload))
  print("done")
...
@app.route("/<string:userid>/dashboard/")
def dashboard(userid):
  return render_template("DashBoard.html")

@app.route("/<string:userid>/review/<string:deck_name>")
def review(userid,deck_name):
  return render_template("review.html")

@app.route("/<string:userid>/allwords/<string:deck_name>")
def allwords(userid,deck_name):
...
report.add_argument('url',type=str)
...
class report_web(Resource):
  @auth_required("token")
  def get(self,username):
    user_data=profile_db.query.filter_by(user_name=username).first()
    return {"urldata":user_data.webhook_url},200
  ...
  def post(self,username):
    ...
    if args["url"]!="":
      user_data.webhook_url=args["url"]
      db.session.commit()
      return {"message":"Success"},200
...
api.add_resource(report_web,"/api/<username>/report")
